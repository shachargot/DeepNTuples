...
//  data.add<int>("fj_labelLegacy", 0);
  ...
  data.add<float>("fj_gendau3_pt", 0);
  data.add<float>("fj_gendau3_eta", 0);
  data.add<float>("fj_gendau3_phi", 0);
  data.add<float>("fj_gendau3_mass", 0);
  data.add<float>("fj_gendau3_pid", 0);
  data.add<float>("fj_gendau3_deltaR", 999);
...
bool FatJetInfoFiller::fill(const pat::Jet& jet, size_t jetidx, const JetHelper& jet_helper) {
  ...
  data.fill<int>("fj_isUpsilon", fjlabel.rfind("Upsilon_", 0) == 0);
  ...
  data.fill<float>("fj_gendau1_pt", parts_size > 0 ? parts[0]->pt() : -999);
  data.fill<float>("fj_gendau1_eta", parts_size > 0 ? parts[0]->eta() : -999);
  data.fill<float>("fj_gendau1_phi", parts_size > 0 ? parts[0]->phi() : -999);
  data.fill<float>("fj_gendau1_mass", parts_size > 0 ? parts[0]->mass() : 0);
  data.fill<float>("fj_gendau1_pid", parts_size > 0 ? parts[0]->pdgId() : 0);
  data.fill<float>("fj_gendau1_deltaR", parts_size > 0 ? reco::deltaR(jet, parts[0]->p4()) : 999);
  data.fill<float>("fj_gendau2_pt", parts_size > 1 ? parts[1]->pt() : -999);
  data.fill<float>("fj_gendau2_eta", parts_size > 1 ? parts[1]->eta() : -999);
  data.fill<float>("fj_gendau2_phi", parts_size > 1 ? parts[1]->phi() : -999);
  data.fill<float>("fj_gendau2_mass", parts_size > 1 ? parts[1]->mass() : 0);
  data.fill<float>("fj_gendau2_pid", parts_size > 1 ? parts[1]->pdgId() : 0);
  data.fill<float>("fj_gendau2_deltaR", parts_size > 1 ? reco::deltaR(jet, parts[1]->p4()) : 999);
  data.fill<float>("fj_gendau3_pt", parts_size > 2 ? parts[2]->pt() : -999);
  data.fill<float>("fj_gendau3_eta", parts_size > 2 ? parts[2]->eta() : -999);
  data.fill<float>("fj_gendau3_phi", parts_size > 2 ? parts[2]->phi() : -999);
  data.fill<float>("fj_gendau3_mass", parts_size > 2 ? parts[2]->mass() : 0);
  data.fill<float>("fj_gendau3_pid", parts_size > 2 ? parts[2]->pdgId() : 0);
  data.fill<float>("fj_gendau3_deltaR", parts_size > 2 ? reco::deltaR(jet, parts[2]->p4()) : 999);
